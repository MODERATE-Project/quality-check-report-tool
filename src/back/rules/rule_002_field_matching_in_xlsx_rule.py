    ...
    def validate(self, epc: "EpcDto") -> Dict:
        ...
        if self.MUNICIPIO_FIELD_NAME not in excel_data.columns or self.ZONA_CLIMATICA_FIELD_NAME not in excel_data.columns:
            return {
                "status": "error",
                "rule_id": self.id,
                "message": "El archivo Excel no contiene las columnas 'MUNICIPIO' y 'ZONA_CLIMATICA'."
            }
        ...
        if valid_values.empty:
            return {
                "status": "error",
                "rule_id": self.id,
                "message": f"No se encontraron valores válidos para el municipio '{dependent_value}'."
            }
        ...
        if value_to_validate.strip().lower() not in valid_zones:
            return {
                "status": "error",
                "rule_id": self.id,
                "message": f"El valor '{value_to_validate}' no es válido para el municipio '{dependent_value}'."
            }
        ...
        return {
            "status": "success",
            "rule_id": self.id,
            "message": f"El valor '{value_to_validate}' es válido para el municipio '{dependent_value}'."
        }
