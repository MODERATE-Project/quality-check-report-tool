...
from rules.rule_008_alcance_informacion_xml_rule import AlcanceInformacionXMLRule
...
# Buscar la regla de tipo 'AlcanceInformacionXMLRule'
rule_data = next(
    (rule for rule in cache_data["rules"]["common_rules"] if rule["class"] == "AlcanceInformacionXMLRule"),
    None
)

if not rule_data:
    raise ValueError("No se encontró una regla de tipo 'AlcanceInformacionXMLRule' en el JSON de caché.")

# Instanciar la regla
rule = AlcanceInformacionXMLRule(rule_data)
