...
from rules.rule_016_datos_energeticos_mejora_rule_multilang import DatosEnergeticosMejoraRule
...
rule_data = next(
    (rule for rule in cache_data["rules"]["common_rules"] if rule["class"] == "DatosEnergeticosMejoraRule"),
    None
)

if not rule_data:
    raise ValueError("No se encontró la regla 'DatosEnergeticosMejoraRule' en el JSON de caché.")

rule = DatosEnergeticosMejoraRule(rule_data)
