...
from rules.rule_018_descripcion_visita_rule import DescripcionVisitaRule
...
rule_data = next(
    (rule for rule in cache_data["rules"]["common_rules"] if rule["class"] == "DescripcionVisitaRule"),
    None
)

if not rule_data:
    raise ValueError("No se encontró la regla 'DescripcionVisitaRule' en el JSON de caché.")

rule = DescripcionVisitaRule(rule_data)
