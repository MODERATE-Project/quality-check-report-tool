...
from rules.rule_019_transmitancia_elementos_opacos_rule_multilang import TransmitanciaElementosOpacosRule
...
rule_data = next(
    (rule for rule in cache_data["rules"]["common_rules"]
     if rule["class"] == "TransmitanciaElementosOpacosRule"),
    None
)

if not rule_data:
    raise ValueError("No se encontró la regla 'TransmitanciaElementosOpacosRule' en el JSON de caché.")

# ───────────────────────── ejecución de la regla ───────────────────────
rule = TransmitanciaElementosOpacosRule(rule_data)
