...
from rules.rule_021_factor_solar_huecos_rule_multilang import FactorSolarHuecosRule
...
rule_data = next(
    (rule for rule in cache_data["rules"]["common_rules"]
     if rule["class"] == "FactorSolarHuecosRule"),
    None
)

if not rule_data:
    raise ValueError("No se encontró la regla 'FactorSolarHuecosRule' en el JSON de caché.")

# ───────────────────────── ejecución de la regla ───────────────────────
rule = FactorSolarHuecosRule(rule_data)
