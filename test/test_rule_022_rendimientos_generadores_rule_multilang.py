...
from rules.rule_022_rendimientos_generadores_rule_multilang import RendimientosGeneradoresRule
...
rule_data = next(
    (rule for rule in cache_data["rules"]["common_rules"]
     if rule["class"] == "RendimientosGeneradoresRule"),
    None
)

if not rule_data:
    raise ValueError("No se encontró la regla 'RendimientosGeneradoresRule' en el JSON de caché.")

# ───────────────────────── ejecución de la regla ───────────────────────
rule = RendimientosGeneradoresRule(rule_data)
