...
from rules.rule_023_rendimientos_generadores_refrigeracion_rule_multilang import RendimientosGeneradoresRefrigeracionRule
...
rule_data = next(
    (rule for rule in cache_data["rules"]["common_rules"]
     if rule["class"] == "RendimientosGeneradoresRefrigeracionRule"),
    None
)

if not rule_data:
    raise ValueError("No se encontró la regla 'RendimientosGeneradoresRefrigeracionRule' en el JSON de caché.")

# ───────────────────────── ejecución de la regla ───────────────────────
rule = RendimientosGeneradoresRefrigeracionRule(rule_data)
