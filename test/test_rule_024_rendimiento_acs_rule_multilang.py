...
from rules.rule_024_rendimiento_acs_rule_multilang import RendimientoACSRule
...
rule_data = next(
    (rule for rule in cache_data["rules"]["common_rules"]
     if rule["class"] == "RendimientoACSRule"),
    None
)

if not rule_data:
    raise ValueError("No se encontró la regla 'RendimientoACSRule' en el JSON de caché.")

# ───────────────────────── ejecución de la regla ───────────────────────
rule = RendimientoACSRule(rule_data)
